...
<div class="address">
    <div class="wrapper clearfix"><a href="indexselect">首页</a><span>/</span>
        <c:if test="${title.cate_parent_id == 0}">
            <a href="selectproductlist?fid=${title.cate_id}">${title.cate_name}</a>
        </c:if>
        <c:if test="${title.cate_parent_id != 0}">
            <a href="selectproductlist?cid=${title.cate_id}">${title.cate_name}</a>
        </c:if>
...
<ul class="proList wrapper clearfix">
    ...
    <c:forEach var="p" items="${list}">
        <li><a href="selectproductview?id=${p.product_id}">
            <dl>
                <dt><img src="${p.product_filename}"></dt>
                <dd>【最家】${p.product_name}</dd>
